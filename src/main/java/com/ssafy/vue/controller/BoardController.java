...
public class BoardController {
	...
	@ApiOperation(value = "댓글작성", notes = "댓글작성.", response = String.class)
	@PostMapping("/comment")
	public ResponseEntity<String> comment(@RequestBody @ApiParam(value = "댓글내용", required = true) CommentDto commentDto) throws Exception {
		logger.info("comment - 호출");
		if (commentService.comment(commentDto)) {
			return new ResponseEntity<String>(SUCCESS, HttpStatus.OK);
		}
		return new ResponseEntity<String>(FAIL, HttpStatus.NO_CONTENT);
	}
	
	@ApiOperation(value = "댓글보기", notes = "글번호에 해당하는 댓글을 반환한다.", response = BoardDto.class)
	@GetMapping("/comment/{articleno}")
	public ResponseEntity<List<CommentDto>> getComments(@PathVariable("articleno") @ApiParam(value = "얻어올 글의 글번호.", required = true) int articleno) throws Exception {
		logger.info("getComments - 호출 : " + articleno);
		commentService.getComments(articleno);
